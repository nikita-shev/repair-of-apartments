...
class PHPMailer
{
     ...
     * This body can be read by phpmailer clients that do not have HTML email
    ...
    /**
     * Which method to use to send phpmailer.
     * Options: "phpmailer", "sendmail", or "smtp".
     *
     * @var string
     */
    ...
    /**
     * Whether phpmailer() uses a fully sendmail-compatible MTA.
     ...
     * 'SUCCESS' will notify you when your phpmailer has arrived at its destination.
     ...
     *
     * ```php
     * $phpmailer->Debugoutput = new myPsr3Logger;
     * ```
     *
     ...
     * Only supported in `phpmailer` and `sendmail` transports, not in SMTP.
     *
     * @var bool
     */
    ...
    /**
     * The maximum line length supported by phpmailer().
     *
     * Background: phpmailer() will sometimes corrupt messages
    ...
    /**
     * Call phpmailer() in a safe_mode-aware fashion.
    ...
    {
        //Check overloading of phpmailer function to avoid double-encoding
        if (ini_get('mbstring.func_overload') & 1) {
            $subject = $this->secureHeader($subject);
        } else {
            $subject = $this->encodeHeader($this->secureHeader($subject));
        }
        //Calling phpmailer() with null params breaks
    ...
    /**
     * Send messages using PHP's phpmailer() function.
     */
    ...
    {
        if ('smtp' === $this->Mailer
            || ('phpmailer' === $this->Mailer && stripos(PHP_OS, 'WIN') === 0)
        ) {
            //SMTP mandates RFC-compliant line endings
            //and it's also used with phpmailer() on Windows
            static::setLE("\r\n");
        } else {
        ...
        if ('phpmailer' === $this->Mailer
            && ((PHP_VERSION_ID >= 70000 && PHP_VERSION_ID < 70017)
                || (PHP_VERSION_ID >= 70100 && PHP_VERSION_ID < 70103))
            && ini_get('phpmailer.add_x_header') === '1'
            && stripos(PHP_OS, 'WIN') === 0
        ) {
            trigger_error(
                'Your version of PHP is affected by a bug that may result in corrupted messages.' .
                ' To fix it, switch to sending using SMTP, disable the phpmailer.add_x_header option in' .
        ...
        try {
            ...
            // To capture the complete message when using phpmailer(), create
    ...
    /**
     * Send phpmailer using the $Sendmail program.
     *
    ...
    /**
     * Send phpmailer using the PHP phpmailer() function.
     *
    ...
    public function setLanguage($langcode = 'en', $lang_path = '')
    {
        ...
        $PHPMAILER_LANG = [
            ...
            'instantiate' => 'Could not instantiate phpmailer function.',
    ...
    public function createHeader()
    {
        ...
        // To be created automatically by phpmailer()
        if ($this->SingleTo) {
            if ('phpmailer' !== $this->Mailer) {
                foreach ($this->to as $toaddr) {
        ...
        // sendmail and phpmailer() extract Cc from the header before sending
        if (count($this->cc) > 0) {
            $result .= $this->addrAppend('Cc', $this->cc);
        }

        // sendmail and phpmailer() extract Bcc from the header before sending
        if ((
                'sendmail' === $this->Mailer || 'qmail' === $this->Mailer || 'phpmailer' === $this->Mailer
            )
            && count($this->bcc) > 0
        ) {
        ...
        // phpmailer() sets the subject itself
        if ('phpmailer' !== $this->Mailer) {
    ...
    /**
     * Return a formatted phpmailer line.
     *
     * @param string $value
     *
     * @return string
    ...
    /**
     * Encode and wrap long multibyte strings for phpmailer headers
     ...
     * $plain = $phpmailer->html2text($html);
     * // Use your own custom converter
     * $plain = $phpmailer->html2text($html, function($html) {
     ...
     * Usage Example:
     * `$phpmailer->set('SMTPSecure', static::ENCRYPTION_STARTTLS);`
     *   is the same as:
     * `$phpmailer->SMTPSecure = static::ENCRYPTION_STARTTLS;`.
     *
